...
export function RevenueAnalyticsView({ data, isLoading }: RevenueAnalyticsViewProps) {
  ...
  const { financialSummary, channelDistribution, paymentMethodDistribution, branchRevenueSummary } = data;
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="space-y-3">
            {paymentMethodDistribution.map((pm) => (
              <div key={pm.method} className="flex items-center justify-between p-3 rounded-lg bg-muted/40 text-xs">
                <span className="font-semibold text-foreground">{pm.method}</span>
                <div className="text-right">
                  <div className="font-bold text-foreground">₹{pm.volume.toLocaleString('en-IN')}</div>
