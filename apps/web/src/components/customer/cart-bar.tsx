...
export function CartBar({ token }: { token: string }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#26313C] bg-[#111820]/95 p-4 backdrop-blur">
      <Link
        href={`/t/${token}/cart`}
        className="mx-auto flex w-full max-w-sm items-center justify-between gap-3 rounded-xl bg-[#2AFEB7] px-4 py-3 text-[#0B0F14] shadow-lg transition-all hover:bg-[#22E5A4] active:scale-[0.99]"
      >
        <span className="relative flex items-center gap-2">
          <span className="text-lg leading-none">🛒</span>
          <span className="absolute -right-3 -top-2 flex h-5 min-w-5 items-center justify-center rounded-full bg-[#0B0F14] px-1 text-[10px] font-bold text-[#2AFEB7]">
            {totalQuantity}
          </span>
        </span>
