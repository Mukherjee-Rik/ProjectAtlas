...
export function ForecastExplainabilityCard({ explanation }: ForecastExplainabilityCardProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-6 shadow-xs space-y-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`inline-flex items-center gap-0.5 px-2 py-0.5 rounded text-xs font-bold ${
            deltaPercentage >= 0 ? 'bg-emerald-500/10 text-emerald-600' : 'bg-rose-500/10 text-rose-600'
          }`}>
      ...
      <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 text-xs font-medium text-foreground leading-relaxed">
        {summaryText}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {factors.map((f, idx) => (
          <div
            key={idx}
            className="p-4 rounded-xl border border-border/80 bg-muted/20 flex flex-col justify-between space-y-2"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-foreground line-clamp-1">{f.name}</span>
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${
                f.impactPercentage >= 0 ? 'bg-emerald-500/10 text-emerald-600' : 'bg-rose-500/10 text-rose-600'
              }`}>
