...
export function AIAssistantDrawer({ isOpen, onClose }: AIAssistantDrawerProps) {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      setError(err?.message || err?.error || 'Kafei AI is temporarily unavailable. Please try again.');
    } finally {
        ...
        <div className="w-screen max-w-md bg-card border-l border-border shadow-2xl flex flex-col">
          ...
          <div className="p-4 border-b border-border flex items-center justify-between bg-secondary/30">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold text-foreground">Kafei AI Assistant</h3>
