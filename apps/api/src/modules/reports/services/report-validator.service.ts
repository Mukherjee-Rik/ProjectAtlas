  ...
  validate(dataSource: string, config: ReportConfigurationDto): void {
    ...
    if (!Object.values(DATA_SOURCES).includes(dataSource as any)) {
      throw new BadRequestException(
        `Invalid dataSource: "${dataSource}". Must be one of: ${Object.values(DATA_SOURCES).join(', ')}`,
      );
    }
    ...
    config.metrics.forEach((metricKey) => {
      if (!APPROVED_METRICS[metricKey]) {
        throw new BadRequestException(
          `Invalid metric "${metricKey}". Not found in centralized registry.`,
        );
      }
    });
    ...
    if (config.dimensions) {
      config.dimensions.forEach((dimKey) => {
        if (!APPROVED_DIMENSIONS[dimKey as keyof typeof APPROVED_DIMENSIONS]) {
          throw new BadRequestException(
            `Invalid dimension "${dimKey}". Not found in centralized registry.`,
          );
        }
      });
    }
    ...
    if (config.filters) {
      config.filters.forEach((f) => {
        ...
        if (
          valStr.includes(';') ||
          valStr.toLowerCase().includes('drop table') ||
          valStr.toLowerCase().includes('--')
        ) {
          throw new BadRequestException(
            'Malicious characters detected in filter value',
          );
        }
      });
    }
    ...
    if (
      ...
      !Object.values(VISUALIZATION_TYPES).includes(
        config.visualization.type as any,
      )
    ) {
      throw new BadRequestException(
        `Invalid visualization type "${config.visualization?.type}"`,
      );
    }
    ...
    if (
      ...
      (config.limit < 1 || config.limit > 1000)
    ) {
