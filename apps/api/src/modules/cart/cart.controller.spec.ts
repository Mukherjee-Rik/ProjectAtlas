...
describe('CartController', () => {
  ...
  it('passes quantity updates through with the cart item id', async () => {
    ...
    expect(cartService.updateItemQuantity).toHaveBeenCalledWith(TOKEN, 'ci-1', {
      quantity: 3,
    });
  });
  ...
  describe('AddCartItemDto validation', () => {
    it('accepts a valid payload', async () => {
      expect(
        await errorsFor(AddCartItemDto, {
          menuItemId: MENU_ITEM_ID,
          quantity: 2,
          variantIds: [],
          addonIds: [],
    ...
    it('accepts a payload without a quantity', async () => {
      expect(
        await errorsFor(AddCartItemDto, { menuItemId: MENU_ITEM_ID }),
      ).toHaveLength(0);
    });

    // Test 9
    it('Test 9: rejects quantity 0', async () => {
      const errors = await errorsFor(AddCartItemDto, {
        menuItemId: MENU_ITEM_ID,
        quantity: 0,
      });
  ...
  describe('UpdateCartItemDto validation', () => {
    it('accepts a quantity inside the 1-99 range', async () => {
      expect(await errorsFor(UpdateCartItemDto, { quantity: 3 })).toHaveLength(
        0,
      );
      expect(await errorsFor(UpdateCartItemDto, { quantity: 99 })).toHaveLength(
        0,
      );
    });
    ...
    it('rejects quantity 0 and 100', async () => {
      expect(
        await errorsFor(UpdateCartItemDto, { quantity: 0 }),
      ).not.toHaveLength(0);
      expect(
        await errorsFor(UpdateCartItemDto, { quantity: 100 }),
      ).not.toHaveLength(0);
    });
  ...
  describe('SetCartItemDto validation', () => {
    it('accepts quantity 0 (for item removal) and positive quantities up to 99', async () => {
      expect(
        await errorsFor(SetCartItemDto, {
          menuItemId: MENU_ITEM_ID,
          quantity: 0,
        }),
      ).toHaveLength(0);
