...
export class MenuItemVariantsService {
  ...
  private async verifyGroupOwnership(groupId: string, restaurantId: string) {
    const group = await this.prisma.menuItemVariantGroup.findFirst({
      where: {
        id: groupId,
    ...
    if (!group)
      throw new ForbiddenException(
        'Variant group does not belong to your active restaurant',
      );
    return group;
  }
  ...
  private async verifyVariantOwnership(
    ...
    if (!variant)
      throw new ForbiddenException(
        'Variant does not belong to your active restaurant',
      );
    return variant;
  }
  ...
  async removeGroup(restaurantId: string, groupId: string) {
    await this.verifyGroupOwnership(groupId, restaurantId);
    return this.prisma.menuItemVariantGroup.delete({
      where: { id: groupId },
