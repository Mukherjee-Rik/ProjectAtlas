...
export class ForecastExplainabilityService {
  ...
  async explainForecast(
    ...
    const avgPrev7 =
      prev7.length > 0
        ? prev7.reduce((s, h) => s + h.grossSales, 0) / prev7.length
        : baseSales;
    const momentumPct =
      avgPrev7 > 0
    ...
    const factors: ExplainabilityFactor[] = [
      ...
      {
        name: isWeekend
          ? 'Weekend Rush Concentration'
          : 'Midweek Lunch/Dinner Distribution',
      ...
      },
      {
        ...
        impactType: 'NEGATIVE',
        description:
          'Estimated historical margin buffer for order cancellations and minor refunds.',
      },
    ];
