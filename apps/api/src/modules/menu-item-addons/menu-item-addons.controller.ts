...
export class MenuItemAddonsController {
  ...
  @Post('menu-items/:itemId/addon-groups') @Permissions(PERMISSIONS.MENU_ITEM_ADDONS_CREATE)
  async createGroup(@Param('itemId', new ParseUUIDPipe()) itemId: string, @CurrentRestaurant() r: CurrentRestaurantType, @Body() dto: CreateAddonGroupDto) {
    if (!r) throw new BadRequestException('No active restaurant selected');
    return this.service.createGroup(r.id, itemId, dto);
  }

  @Get('menu-items/:itemId/addon-groups') @Permissions(PERMISSIONS.MENU_ITEMS_READ)
  ...
  @Patch('addon-groups/:id') @Permissions(PERMISSIONS.MENU_ITEM_ADDONS_UPDATE)
  async updateGroup(@Param('id', new ParseUUIDPipe()) id: string, @CurrentRestaurant() r: CurrentRestaurantType, @Body() dto: UpdateAddonGroupDto) {
    if (!r) throw new BadRequestException('No active restaurant selected');
    return this.service.updateGroup(r.id, id, dto);
  }

  @Delete('addon-groups/:id') @Permissions(PERMISSIONS.MENU_ITEM_ADDONS_DELETE)
  ...
  @Post('addon-groups/:groupId/addons') @Permissions(PERMISSIONS.MENU_ITEM_ADDONS_CREATE)
  async createAddon(@Param('groupId', new ParseUUIDPipe()) groupId: string, @CurrentRestaurant() r: CurrentRestaurantType, @Body() dto: CreateAddonDto) {
    if (!r) throw new BadRequestException('No active restaurant selected');
    return this.service.createAddon(r.id, groupId, dto);
  }

  @Patch('addons/:id') @Permissions(PERMISSIONS.MENU_ITEM_ADDONS_UPDATE)
  async updateAddon(@Param('id', new ParseUUIDPipe()) id: string, @CurrentRestaurant() r: CurrentRestaurantType, @Body() dto: UpdateAddonDto) {
    if (!r) throw new BadRequestException('No active restaurant selected');
    return this.service.updateAddon(r.id, id, dto);
  }

  @Delete('addons/:id') @Permissions(PERMISSIONS.MENU_ITEM_ADDONS_DELETE)
  async removeAddon(@Param('id', new ParseUUIDPipe()) id: string, @CurrentRestaurant() r: CurrentRestaurantType) {
    if (!r) throw new BadRequestException('No active restaurant selected');
