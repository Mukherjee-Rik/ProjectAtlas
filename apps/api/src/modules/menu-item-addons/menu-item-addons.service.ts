...
export class MenuItemAddonsService {
  ...
  private async verifyGroupOwnership(groupId: string, restaurantId: string) {
    const group = await this.prisma.menuItemAddonGroup.findFirst({ where: { id: groupId, menuItem: { category: { menu: { restaurantId } } } } });
    if (!group) throw new ForbiddenException('Addon group does not belong to your active restaurant');
  }

  private async verifyAddonOwnership(addonId: string, restaurantId: string) {
    const addon = await this.prisma.menuItemAddon.findFirst({ where: { id: addonId, group: { menuItem: { category: { menu: { restaurantId } } } } } });
    if (!addon) throw new ForbiddenException('Addon does not belong to your active restaurant');
  }

  async createGroup(restaurantId: string, menuItemId: string, dto: CreateAddonGroupDto) {
    await this.verifyItemOwnership(menuItemId, restaurantId);
    return this.prisma.menuItemAddonGroup.create({
      data: { menuItemId, name: dto.name, ...(dto.required !== undefined && { required: dto.required }), ...(dto.minSelect !== undefined && { minSelect: dto.minSelect }), ...(dto.maxSelect !== undefined && { maxSelect: dto.maxSelect }), ...(dto.position !== undefined && { position: dto.position }) },
      select: { id: true, menuItemId: true, name: true, required: true, minSelect: true, maxSelect: true, position: true, addons: { select: { id: true, name: true, price: true, position: true, status: true }, orderBy: { position: 'asc' } } },
    });
  }
  ...
  async updateGroup(restaurantId: string, groupId: string, dto: UpdateAddonGroupDto) {
    await this.verifyGroupOwnership(groupId, restaurantId);
    return this.prisma.menuItemAddonGroup.update({
      where: { id: groupId },
      data: { ...(dto.name !== undefined && { name: dto.name }), ...(dto.required !== undefined && { required: dto.required }), ...(dto.minSelect !== undefined && { minSelect: dto.minSelect }), ...(dto.maxSelect !== undefined && { maxSelect: dto.maxSelect }), ...(dto.position !== undefined && { position: dto.position }) },
      select: { id: true, menuItemId: true, name: true, required: true, minSelect: true, maxSelect: true, position: true },
    });
  }

  async removeGroup(restaurantId: string, groupId: string) {
    await this.verifyGroupOwnership(groupId, restaurantId);
    return this.prisma.menuItemAddonGroup.delete({ where: { id: groupId }, select: { id: true, name: true } });
  }
  ...
  async removeAddon(restaurantId: string, addonId: string) {
    await this.verifyAddonOwnership(addonId, restaurantId);
    return this.prisma.menuItemAddon.delete({ where: { id: addonId }, select: { id: true, name: true } });
  }
}
