...
export class MenuItemsService {
  ...
  async create(restaurantId: string, createDto: CreateMenuItemDto) {
    ...
    if (!category)
      throw new ForbiddenException(
        'Category does not belong to your active restaurant',
      );

    if (taxRateId) {
      const taxRate = await this.prisma.taxRate.findFirst({
        where: { id: taxRateId, restaurantId },
      });
      if (!taxRate)
        throw new ForbiddenException(
          'Tax rate does not belong to your active restaurant',
        );
    }
    ...
    if (existing)
      throw new ConflictException(
        'An item with this code already exists in this category',
      );
  ...
  async update(id: string, restaurantId: string, updateDto: UpdateMenuItemDto) {
    ...
    if (!existing)
      throw new ForbiddenException(
        'Menu item not found or does not belong to active restaurant',
      );
    ...
    if (categoryId && categoryId !== existing.categoryId) {
      ...
      if (!category)
        throw new ForbiddenException(
          'Target category does not belong to active restaurant',
        );
    }
    ...
    if (
      code &&
      (code !== existing.code || targetCategoryId !== existing.categoryId)
    ) {
      const duplicate = await this.prisma.menuItem.findUnique({
        where: { categoryId_code: { categoryId: targetCategoryId, code } },
      });
      if (duplicate)
        throw new ConflictException(
          'Another item with this code already exists in this category',
        );
    }
